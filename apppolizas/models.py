...
class Usuario(AbstractUser):
    ...
    rol = models.CharField(
        max_length=20, choices=TIPO_USUARIO_CHOICES, default=ANALISTA
    )
...
class ResponsableCustodio(models.Model):
    ...
    departamento = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="Departamento/Carrera"
    )

    # Campos nuevos agregados según la imagen del Acta
    ciudad = models.CharField(
        max_length=100, blank=True, null=True, help_text="Ej: Loja - CPL"
    )
    edificio = models.CharField(
        max_length=100, blank=True, null=True, help_text="Ej: Edificio D"
    )
    puesto = models.CharField(
        max_length=150, blank=True, null=True, help_text="Ej: D4D06-6-PUESTO DE DOCENTE"
    )
...
class Bien(models.Model):
    ...
    baan_v = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="BAAN V"
    )
    ...
    modelo = models.CharField(
        max_length=100, blank=True, null=True, default="No aplica"
    )
    marca = models.CharField(max_length=100, blank=True, null=True)
    ubicacion = models.CharField(
        max_length=100, default="Sala H - Computo", verbose_name="Ubicación"
    )
...
class Poliza(models.Model):
    ...
    aseguradora = models.ForeignKey(
        Aseguradora, on_delete=models.PROTECT, related_name="polizas"
    )
...
class DocumentoPoliza(models.Model):
    poliza = models.ForeignKey(
        Poliza, related_name="documentos", on_delete=models.CASCADE
    )
...
class Siniestro(models.Model):
    ...
    poliza = models.ForeignKey(
        Poliza, on_delete=models.CASCADE, related_name="siniestros"
    )
    ...
    ubicacion_bien = models.CharField(
        max_length=255, help_text="Lugar exacto donde ocurrió el siniestro"
    )
    ...
    estado_tramite = models.CharField(
        max_length=50, choices=ESTADO_CHOICES, default="REPORTADO"
    )
    ...
    valor_reclamo_estimado = models.DecimalField(
    ...
    def clean(self):
        ...
        if self.bien.custodio != self.custodio:
            raise ValidationError(
                f"El bien '{self.bien.detalle}' no pertenece al custodio {self.custodio.nombre_completo}."
            )
...
class Finiquito(models.Model):
    siniestro = models.OneToOneField(
        Siniestro, on_delete=models.CASCADE, related_name="finiquito"
    )
    ...
    valor_total_reclamo = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Monto bruto reclamado"
    )
    valor_deducible = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Deducible aplicado"
    )
    valor_depreciacion = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="Depreciación del bien"
    )
    valor_final_pago = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Valor líquido a recibir"
    )
...
class Factura(models.Model):
    poliza = models.ForeignKey(
        Poliza, on_delete=models.CASCADE, related_name="facturas"
    )
    ...
    prima = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Valor de la prima"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        self.base_imponible = (
            self.prima
            + self.contribucion_super
            + self.seguro_campesino
            + self.derechos_emision
        )
        ...
        self.valor_a_pagar = (
            self.total_facturado - self.retenciones - self.descuento_pronto_pago
        )

        if self.pagado:
            self.mensaje_resultado = "Pagado"
        elif self.valor_a_pagar <= 0:
            self.mensaje_resultado = "Saldado"
        else:
            self.mensaje_resultado = "Pendiente"
...
class DocumentoSiniestro(models.Model):
    ...
    siniestro = models.ForeignKey(
        Siniestro, on_delete=models.CASCADE, related_name="documentos"
    )
...
# ========================================================
# 9. SISTEMA DE REPORTES EXTERNOS
# ========================================================
